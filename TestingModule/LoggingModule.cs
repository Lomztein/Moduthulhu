...
using Lomztein.Moduthulhu.Modules.Meta;
using System;

namespace Lomztein.Moduthulhu.Modules.Misc.Logging {

    [Dependency ("ModuleManagerModule")]
    public class LoggingModule : ModuleBase {
        ...
        public override void Initialize() {
            ...
            ParentShard.ChannelUpdated += OnChannelUpdated;
            ParentShard.GuildAvailable += OnGuildAvailable;
            ParentShard.GuildMembersDownloaded += OnGuildMembersDownloaded;
            ParentShard.GuildMemberUpdated += OnGuildMemberUpdated;
            ParentShard.GuildUnavailable += OnGuildUnavailable;
            ParentShard.GuildUpdated += OnGuildUpdated;
            ParentShard.JoinedGuild += OnJoinedGuild;
            ParentShard.LeftGuild += OnLeftGuild;
            ParentShard.MessageDeleted += OnMessageDeleted;
            ParentShard.MessageReceived += OnMessageRecieved;
            ParentShard.MessageUpdated += OnMessageUpdated;
            ParentShard.ReactionAdded += OnReactionAdded;
            ParentShard.ReactionRemoved += OnReactionRemoved;
            ParentShard.ReactionsCleared += OnReactionsCleared;
            ParentShard.RoleCreated += OnRoleCreated;
            ParentShard.RoleDeleted += OnRoleDeleted;
            ParentShard.RoleUpdated += OnRoleUpdated;
            ParentShard.UserBanned += OnUserBanned;
            ParentShard.UserJoined += OnUserJoined;
            ParentShard.UserLeft += OnUserLeft;
            ParentShard.UserUnbanned += OnUserUnbanned;
            ParentShard.UserUpdated += OnUserUpdated;
        ...
        private Task OnMessageRecieved(SocketMessage arg) {
            ...
            throw new DivideByZeroException ("I cannot allow you to do that, sir.");
            return Task.CompletedTask;
        }
